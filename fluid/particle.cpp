...
void incremento_densidades (double h, Particula &particula_i, Particula &particula_j) {
    double const norma = std::pow(particula_i.px - particula_j.px, 2) + std::pow(particula_i.py - particula_j.py, 2) +
...
void incremento_aceleracion (Particula &particula_i, Particula &particula_j, double h, double norma, double m){
    double const operador_izquierda = 15 * m * std::pow(h - std::pow(std::max(norma,
        std::pow(10, -12)), 0.5), 2) * (particula_i.densidad + particula_j.densidad -
                2 * (std::pow(10, -3))) / (3.14 * std::pow(h, 6) *
...
void Cubo{
    ...
    void collision_x(class particula, double xmin, double xmax, double h ){//o mejor llamar a la funcion nx aqui//
        ...
        if (c_x=0){
            ...
            if incremento>dp{
                        particula.ac_x+= cs*incremento_x-dv*particula.vx
                };
        }
        if (c_x= nx-1){
    ...
    void collision_y(class particula, double ymin, double ymax, double h ){//o mejor llamar a la funcion nx aqui//
        ...
        if (c_x=0){
            ...
            if incremento>dp{
                        particula.ac_y+= cs*incremento_y-dv*particula.vy
                };
        }
        if (c_y= ny-1){
