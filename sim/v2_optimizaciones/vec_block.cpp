...
void Vec_Bloque::colision_x(int n_x, std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    ...
    if (incremento_x > limite_colision) {
      ...
      if (n_x != 0) {
        aumento = -s_c * incremento_x - d_v * Lista_b[i_d].vxyz[0];
      }
...
void Vec_Bloque::colision_y(int n_y, std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    ...
    if (incremento_y > limite_colision) {
      ...
      if (n_y != 0) {
        aumento = -s_c * incremento_y - d_v * Lista_b[i_d].vxyz[1];
      }
...
void Vec_Bloque::colision_z(int n_z, std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    ...
    if (incremento_z > limite_colision) {
      ...
      if (n_z != 0) {
        aumento = -s_c * incremento_z - d_v * Lista_b[i_d].vxyz[2];
      }
...
void Vec_Bloque::colision_x_baja(std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    double const const_x      = Lista_b[i_d].pxyz[0] + Lista_b[i_d].hvxyz[0] * a_tiempo;
    double const incremento_x = d_p - const_x + min[0];
    if (incremento_x > limite_colision) {
      Lista_b[i_d].a_c[0] += s_c * incremento_x - d_v * Lista_b[i_d].vxyz[0];
    }
    Lista_b[i_d].movimiento_particulas();
    double const d_x = Lista_b[i_d].pxyz[0] - min[0];
    if (d_x < 0) { Lista_b[i_d].pxyz[0] = min[0] - d_x; }
    Lista_b[i_d].vxyz[0]  = -Lista_b[i_d].vxyz[0];
    Lista_b[i_d].hvxyz[0] = -Lista_b[i_d].hvxyz[0];
  }
}

void Vec_Bloque::colision_y_baja(std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    double const const_y      = Lista_b[i_d].pxyz[1] + Lista_b[i_d].hvxyz[1] * a_tiempo;
    double const incremento_y = d_p - const_y + min[1];
    if (incremento_y > limite_colision) {
      Lista_b[i_d].a_c[0] += s_c * incremento_y - d_v * Lista_b[i_d].vxyz[1];
    }
    Lista_b[i_d].movimiento_particulas();
    double const d_y = Lista_b[i_d].pxyz[1] - min[1];
    if (d_y < 0) { Lista_b[i_d].pxyz[1] = min[1] - d_y; }
    Lista_b[i_d].vxyz[1]  = -Lista_b[i_d].vxyz[1];
    Lista_b[i_d].hvxyz[1] = -Lista_b[i_d].hvxyz[1];
  }
}

void Vec_Bloque::colision_z_baja(std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    double const const_z =
        Lista_b[i_d].pxyz[2] + Lista_b[i_d].hvxyz[2] * a_tiempo;  // delta de t no va a ser 1 siempre?//
    double const incremento_z = d_p - const_z + min[2];
    if (incremento_z > limite_colision) {
      Lista_b[i_d].a_c[0] += s_c * incremento_z - d_v * Lista_b[i_d].vxyz[2];
    }
    Lista_b[i_d].movimiento_particulas();
    double const d_z = Lista_b[i_d].pxyz[2] - min[2];
    if (d_z < 0) { Lista_b[i_d].pxyz[2] = min[2] - d_z; }
    Lista_b[i_d].vxyz[2]  = -Lista_b[i_d].vxyz[2];
    Lista_b[i_d].hvxyz[2] = -Lista_b[i_d].hvxyz[2];
  }
}

void Vec_Bloque::colision_x_alta(std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    double const const_x =
        Lista_b[i_d].pxyz[0] + Lista_b[i_d].hvxyz[0] * a_tiempo;  // delta de t no va a ser 1 siempre?//
    double const incremento_x = d_p - max[0] + const_x;
    if (incremento_x > limite_colision) {
      Lista_b[i_d].a_c[0] += -s_c * incremento_x + d_v * Lista_b[i_d].vxyz[0];
    }
    Lista_b[i_d].movimiento_particulas();
    double const d_x = max[0] - Lista_b[i_d].pxyz[0];
    if (d_x < 0) { Lista_b[i_d].pxyz[0] = max[0] + d_x; }
    Lista_b[i_d].vxyz[0]  = -Lista_b[i_d].vxyz[0];
    Lista_b[i_d].hvxyz[0] = -Lista_b[i_d].hvxyz[0];
  }
}

void Vec_Bloque::colision_y_alta(std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    double const const_y =
        Lista_b[i_d].pxyz[1] + Lista_b[i_d].hvxyz[1] * a_tiempo;  // delta de t no va a ser 1 siempre?//
    double const incremento_y = d_p - max[1] + const_y;
    if (incremento_y > limite_colision) {
      Lista_b[i_d].a_c[0] += -s_c * incremento_y + d_v * Lista_b[i_d].vxyz[1];
    }
    Lista_b[i_d].movimiento_particulas();
    double const d_y = max[1] - Lista_b[i_d].pxyz[1];
    if (d_y < 0) { Lista_b[i_d].pxyz[1] = max[1] + d_y; }
    Lista_b[i_d].vxyz[1]  = -Lista_b[i_d].vxyz[1];
    Lista_b[i_d].hvxyz[1] = -Lista_b[i_d].hvxyz[1];
  }
}

void Vec_Bloque::colision_z_alta(std::vector<Particula> &Lista_b) {
  for (int i_d : lista_particulas) {
    double const const_z =
        Lista_b[i_d].pxyz[2] + Lista_b[i_d].hvxyz[2] * a_tiempo;  // delta de t no va a ser 1 siempre?//
    ...
    Lista_b[i_d].movimiento_particulas();
    double const d_z = max[2] - Lista_b[i_d].pxyz[2];
    if (d_z < 0) { Lista_b[i_d].pxyz[2] = max[2] + d_z; }
    Lista_b[i_d].vxyz[2]  = -Lista_b[i_d].vxyz[2];
    Lista_b[i_d].hvxyz[2] = -Lista_b[i_d].hvxyz[2];
  }
}
